...
/**
 * @ngdoc function
 * @name ibmApp.directive : Custom Modal Directive
 * @description
...
angular.module('ibmApp')
  .directive("pinDisplay", function(){
    return {
      ...
      scope: {
        xxx: '=imgTransporter',
        modalShower: '&modalOpener'
      },

      link: function(scope, textModifier){
        ...
        scope.showInModal = function(idx){
          ...
          var displayWidth = 0.7 * browseWidth; //width of the image display box should be 70% of the screen size.
          ...
          $(rol_title).css({
            backgroundColor: 'rgb(207, 165, 109)',
            width: '40%',
          ...
          $('.closeX, #roldal').on('click', function() {
            $('#roldal').css('display', 'none');
            $('#roldal_over').css('display', 'none');
